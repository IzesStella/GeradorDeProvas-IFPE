    ...
    <div style={{ backgroundColor: '#f4f6f8', minHeight: '100vh', paddingBottom: '60px', color: '#333' }}>
      <header style={{ backgroundColor: '#1a1b26', color: 'white', padding: '15px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '50px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
          ...
          <button onClick={onVoltar} style={{ backgroundColor: '#2c3e50', color: '#2ecc71', border: 'none', padding: '8px 20px', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}>VOLTAR</button>
        </div>
      </header>
        ...
        {questoes.length === 0 ? (
          ...
          questoes.map((questao, index) => (
            <div key={questao.id} style={{ backgroundColor: '#ffffff', borderRadius: '12px', marginBottom: '35px', padding: '25px 30px', boxShadow: '0 6px 15px rgba(0,0,0,0.08)' }}>
              ...
              <SyntaxHighlighter language="typescript" style={vscDarkPlus} showLineNumbers={true} customStyle={{ borderRadius: '8px', padding: '20px', fontSize: '14px' }}>
                {questao.codigo_typescript}
              </SyntaxHighlighter>
              {questao.easter_egg_conteudo && (
                <div style={{ backgroundColor: '#fdf8e4', border: '1px solid #faebcc', borderRadius: '8px', marginTop: '25px', padding: '15px' }}>
