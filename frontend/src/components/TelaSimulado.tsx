...
export function TelaSimulado({ questoes, onVoltar, filtros }: TelaSimuladoProps) {
  // LÓGICA DO AVISO INTELIGENTE (Faltando Dificuldade)
  ...
  // NOVA LÓGICA: AVISO DE QUANTIDADE INSUFICIENTE NO BANCO
    ...
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, minHeight: '100vh', backgroundColor: '#f4f6f8', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#333' }}>
      ...
      <div className="tela-sistema">
        ...
        <div style={{ maxWidth: '960px', margin: '0 auto', padding: '0 20px' }}>
          ...
          {/* NOVO AVISO: Quantidade Insuficiente no Banco */}
          {deveMostrarAvisoQuantidade && (
            <div style={{ backgroundColor: '#e3f2fd', borderLeft: '5px solid #2196f3', padding: '16px 20px', borderRadius: '6px', marginBottom: '20px', color: '#0d47a1', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {deveMostrarAvisoFaltantes && (
            <div style={{ backgroundColor: '#fff8e1', borderLeft: '5px solid #ffc107', padding: '16px 20px', borderRadius: '6px', marginBottom: '30px', color: '#856404', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                <strong>Aviso de Dificuldade:</strong> Não encontramos questões de nível <strong>{faltantes.join(', ')}</strong> para os tópicos selecionados. <br/>
                Exibindo apenas as dificuldades disponíveis para garantir seu estudo.
              </div>
            </div>
          )}

          {/* AVISO ABSOLUTO: Nenhuma questão encontrada */}
          {questoes.length === 0 ? (
